...
function get_count_orders($pdo, $user_id){
    $get_count_orders= $pdo->prepare("SELECT COUNT(*) FROM zakaz WHERE user_id=:user_id");
    $get_count_orders->bindParam(':user_id', $user_id);
    $get_count_orders->execute();
    $count = $get_count_orders->fetch(PDO::FETCH_NUM);
    return $count[0];
}
